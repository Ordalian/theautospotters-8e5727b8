...
async function listAllObjectPaths(adminClient: any, bucket: string, prefix: string) {
  ...
  while (true) {
    ...
    for (const item of items) {
      // item.name is relative to prefix (we pass userId without trailing slash).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Client bound to caller token to read auth claims + own profile.
    ...
    // Tables cleanup (friendships + DMs have no FK cascade in this schema).
    await adminClient
