-- Leaderboard: exclude miniature spots (hot_wheels) from count and all aggregates
...
AS $$
  SELECT
    ...
    COALESCE(SUM(CASE WHEN c.estimated_price IS NOT NULL AND c.estimated_price > 0 THEN c.estimated_price ELSE 0 END), 0)::numeric AS total_estimated_price
  FROM public.cars c
  LEFT JOIN public.profiles p ON p.user_id = c.user_id
  WHERE (c.vehicle_type IS NULL OR c.vehicle_type <> 'hot_wheels')
