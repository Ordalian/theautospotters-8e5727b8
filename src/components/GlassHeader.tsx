...
export function GlassHeader({ title, subtitle, backTo, rightContent, children }: GlassHeaderProps) {
  ...
  return (
    <header className="glass-header sticky top-0 z-30 px-4 py-3">
      <div className="flex items-center justify-between gap-3 max-w-2xl mx-auto">
        ...
        <div className="flex-1 text-center min-w-0">
          <h1 className="text-lg font-bold text-foreground truncate font-heading">
            {title}
          </h1>
          {subtitle && (
        ...
        <div className="shrink-0 flex items-center gap-1">
          {rightContent ?? <div className="w-5" />}
        </div>
      </div>
      {children}
    </header>
