...
const Dashboard = () => {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { latestCarImage, carCount: cars.length, mapSpots };
    },
    enabled: !!user,
    staleTime: 2 * 60 * 1000,
  });
  ...
  const tiles = [
    { title: "My Garage", subtitle: `${carCount} car${carCount !== 1 ? "s" : ""} spotted`, icon: Car, image: latestCarImage, onClick: () => navigate("/garage"), gradient: "from-primary/20 to-primary/5" },
    { title: "Friends' Garages", subtitle: "See your friends", icon: Users, image: null, onClick: () => navigate("/friends"), gradient: "from-blue-500/20 to-blue-500/5" },
    { title: "The AutoSpotter", subtitle: "AI car recognition", icon: Brain, image: null, onClick: () => navigate("/autospotter"), gradient: "from-emerald-500/20 to-emerald-500/5" },
    { title: "Leaderboard", subtitle: "Top spotters", icon: Trophy, image: null, onClick: () => navigate("/leaderboard"), gradient: "from-amber-500/20 to-amber-500/5" },
  ];
  ...
  return (
    <div className="min-h-screen bg-background relative">
      ...
      <main className="p-6 max-w-2xl mx-auto relative z-10">
        <h2 className="text-lg font-medium text-muted-foreground mb-6">Hey, {user?.email?.split("@")[0]} 👋</h2>
